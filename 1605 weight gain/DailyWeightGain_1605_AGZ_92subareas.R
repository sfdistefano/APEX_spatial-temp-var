...
# Subareas are divided by ecological site for each pasture
# Precipitation input is the same for all subareas
setwd("D:/01-APEX1605_w1905format/APEX1605 for CPER_1605 format")
...
name.agz <- c("SA", "ID", "Year", "Year_num", "CPNM",
              "GNAM", "Herd", "Owner","GZNB", "GZD", "GZSL", "GZSD", "HAY", "HAYd", "WTG",
              "WSAha", "PSTLkg/ha", "PSTDkg/ha", "PSTTkg/ha","POP", "peakbm")
...
PastureID <- read.csv("D:/APEX data and scripts/Data/PastureID_ecosite_92subareas_trt divided.csv")
...
wtg <- fread("D:/APEX data and scripts/Data/CPER Cattle/CARM_Cattle Weight Gains_2014-2022.csv") %>%
  ...
  left_join(PastureID_20sa,by = 'Pasture', relationship = "many-to-many")
...
Gain_AGz <- TGM_AGZ %>%
  ...
  merge(PastureID, by.x = c("ID", "Treatment"), by.y = c("PastureID", "Treatment"), 
        relationship = "many-to-many") %>%
  group_by(Year, Pasture, Treatment) %>%
  summarize(MeanWT_past = mean(WTG)) %>%
  group_by(Year, Treatment) %>%
  summarize(MeanWT = mean(MeanWT_past), stdevWT = 0, Type = "Simulated")
...
#   merge(PastureID,
#         by.x = c("ID", "Treatment"),
#         by.y = c("PastureID", "Treatment")) %>%
#   select(Year, Pasture, Ecosite, Treatment, WTG) %>%
#   group_by(Year, Pasture, Ecosite, Treatment) %>%
#   dplyr::summarise(MeanWT = mean(`WTG`,na.rm = T),
#                    stdevWT = 0, Type = "Simulated") %>%
#   select(Year, Pasture, Treatment, MeanWT, Type) %>%
#   group_by(Year, Pasture, Treatment, Type) %>%
#   dplyr::summarize(MeanWT = mean(MeanWT,na.rm = T),
#                    stdevWT = 0) %>%
#   merge(PastureID_20sa) %>%
#   select(Year, Pasture, Ecosite, Treatment, MeanWT, Type) %>%
#   group_by(Year, Ecosite, Treatment, Type) %>%
#   dplyr::summarize(MeanWT = mean(MeanWT,na.rm = T),
#                    stdevWT = 0)
# 
# ## Combining results
