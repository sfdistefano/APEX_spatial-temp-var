...
noVar <- read.csv("C:/APEX data and scripts/APEX outputs/test_output_compare_noVar_allTRM.csv") %>%
...
spatVar <- read.csv("C:/APEX data and scripts/APEX outputs/test_output_compare_spatVar_allTRM.csv") %>%
  select(APEXcodeFG, MeankgPerHa.x, MeankgPerHa.y) %>%
  rename(Simulated = MeankgPerHa.x, Observed = MeankgPerHa.y) %>%
  mutate(Sim.Type = "Spatial Variability")

spatTempVar <- read.csv("C:/APEX data and scripts/APEX outputs/test_output_compare_spatTempVar_allTRM.csv")%>%
  select(APEXcodeFG, MeankgPerHa.x, MeankgPerHa.y) %>%
  rename(Simulated = MeankgPerHa.x, Observed = MeankgPerHa.y) %>%
  mutate(Sim.Type = "Spatial + Temporal Variability")


sim_obs_compare <- rbind(noVar, spatVar, spatTempVar)
