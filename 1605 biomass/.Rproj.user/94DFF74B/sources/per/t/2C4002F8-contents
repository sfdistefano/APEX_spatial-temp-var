...
biomass_no_variability <- read_preprocess_biomass(
  file_path = "C:/01-APEX1605_CO_baseline/Cage Biomass Simulation/APEX1605_CO_all20/CONUNN_TGM.cag",
  biomass_col = 'AB_DDMkg/ha',
...
# - Filter for years 2014 to 2023 and convert wide data to long format
...
for (group in plant_functional_groups) {
  ...
  plot <- ggplot(biomass_summary_group, aes(x = month_day, y = mean_DDMkg_ha, color = Treatment, group = Treatment)) +
    ...
    geom_point(data = observed_data_group,
               aes(x = month_day, y = MeankgPerHa, color = Treatment, shape = Treatment),
    ...
    labs(
      title = paste("Plant Group:", ifelse(group == "FRB3", "FORB", group)),
      x = ifelse(group %in% c("CSPG", "WSPG"), "", "Date"),  # Use "" instead of NULL
      y = ifelse(group %in% c("WSPG", "VUOC"), "", "Accumulated Biomass (kg/ha)"),  # Use "" instead of NULL
